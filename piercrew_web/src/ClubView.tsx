...
import {ArrowUpRight, Download, Pencil, RefreshCw, Save, Search, ShieldCheck, Trophy, UserPlus, X} from 'lucide-react';
...
export default function ClubView({user,canWrite,onDirty,onLegacy}:{user:User;canWrite:boolean;onDirty:(value:boolean)=>void;onLegacy:()=>void}){
  ...
  function edit(p:ClubPlayer){setForm({id:p.id,version:p.version,name:p.name,team:p.team,potential:p.potential,role:p.role,badge:p.badge});setError('');}
  ...
  async function savePlayer(){
    ...
    setBusy(true);setError('');
    try{const payload={name:form.name.trim(),team:form.team.trim(),potential:form.potential,...(form.badge?{badge:form.badge}:{}),...(admin?{role:form.role}:{})};
  ...
  return <>
    <section className="page-heading"><div><span className="eyebrow">PIERCREW CLUB · ANAGRAFICA E ARCHIVIO</span><h1>Gestione club</h1><p>Giocatori, palmarès e tornei del mondo PierCrew.</p></div><button className="secondary" disabled={busy} onClick={()=>void refresh()}><RefreshCw size={16}/>Aggiorna</button></section>
    ...
    {tab==='players'&&<section className="results-card club-section"><div className="section-heading"><div><h2>Rosa giocatori</h2><p>{players.length} tesserati · potenziale medio {players.length?(players.reduce((n,p)=>n+p.potential,0)/players.length).toFixed(1):'0.0'}/10</p></div><div className="club-actions"><label className="search"><Search size={16}/><input aria-label="Cerca giocatore" value={search} onChange={e=>setSearch(e.target.value)} placeholder="Cerca giocatore…"/></label>{canWrite&&<button className="primary" onClick={create}><UserPlus size={16}/>Nuovo giocatore</button>}</div></div>
      ...
      <div className="club-table-hint">Scorri la tabella verso destra per vedere potenziale, ruolo, accesso e azioni.</div>
      <div className="table-scroll club-table-wrap"><table className="club-table"><thead><tr><th>Giocatore</th><th>Squadra</th><th>Potenziale</th><th>Ruolo</th><th>Accesso</th><th>Azioni</th></tr></thead><tbody>{filteredPlayers.map(p=><tr key={p.id}>
        <td><span className="club-player-name">{canWrite&&<button type="button" className="club-edit-inline" aria-label={`Modifica ${p.name}`} title={`Modifica ${p.name}`} onClick={()=>edit(p)}><Pencil size={15}/></button>}{admin?<input aria-label={`Nome ${p.name}`} value={drafts[p.id]?.name??p.name} onChange={e=>patch(p.id,{name:e.target.value})}/>:p.name}</span></td>
    ...
    {form&&<div className="modal-backdrop"><section className="modal club-modal" role="dialog" aria-modal="true" aria-label={form.id?'Modifica giocatore':'Nuovo giocatore'}><div className="section-heading"><h2>{form.id?'Modifica giocatore':'Nuovo giocatore'}</h2><button aria-label="Chiudi" disabled={busy} onClick={()=>setForm(null)}><X/></button></div><form onSubmit={e=>{e.preventDefault();void savePlayer();}}><label>Nome giocatore<input required maxLength={160} value={form.name} onChange={e=>setForm({...form,name:e.target.value})}/></label><label>Squadra<input maxLength={100} value={form.team} onChange={e=>setForm({...form,team:e.target.value})}/></label><div className="club-badge-choice"><span className="team-mark"><TeamMark name={form.team||form.name} badge={form.badge}/></span><span>{form.badge?.kind==='custom'?'🎨 Stemma personalizzato':form.badge?.kind==='club'?'⚽ Club calcistico':form.badge?.kind==='flag'?'🏳️ Bandiera':'Nessuna immagine'}</span><button type="button" className="secondary compact" onClick={()=>setBadgeEditorOpen(true)}>Scegli immagine</button></div><label>Potenziale<input type="number" min={1} max={10} required value={form.potential} onChange={e=>setForm({...form,potential:Number(e.target.value)})}/></label><label>Ruolo{admin?<select value={form.role} onChange={e=>setForm({...form,role:e.target.value as Role})}>{(['R','W','A'] as Role[]).map(role=><option key={role} value={role}>{roleName[role]}</option>)}</select>:<input value={roleName[form.role]} disabled/>}</label><div className="modal-actions"><button type="button" className="secondary" disabled={busy} onClick={()=>setForm(null)}>Annulla</button><button className="primary" disabled={busy||!form.name.trim()||form.potential<1||form.potential>10}>Salva</button></div></form>{form.id&&admin&&<div className="club-admin-actions"><button className="secondary" disabled={busy} onClick={()=>{const p=players.find(x=>x.id===form.id);if(p)void resetPassword(p);}}>Reset password</button><button className="secondary danger" disabled={busy||form.id===user.id} onClick={()=>{const p=players.find(x=>x.id===form.id);if(p)void deletePlayer(p);}}>Elimina giocatore</button></div>}</section>{badgeEditorOpen&&<BadgeEditor participants={[form.team||form.name||'Nuova squadra']} badges={form.badge?{[form.team||form.name||'Nuova squadra']:form.badge}:{}} busy={busy} onClose={()=>setBadgeEditorOpen(false)} onSave={async selected=>{setForm(current=>current?{...current,badge:selected[form.team||form.name||'Nuova squadra']}:current);setBadgeEditorOpen(false);}}/>}</div>}
