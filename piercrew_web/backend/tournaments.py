...
def view(doc):
    ...
    standings = aggiorna_classifica(frame, doc.get('_piercrew_withdrawals', []))
    return {'id': str(doc['_id']), 'name': doc['nome_torneo'], 'version': version(doc), 'matches': rows,
            ...
            'withdrawals': doc.get('_piercrew_withdrawals', []),
            'closed': bool(doc.get('_piercrew_closed', False)) or doc['nome_torneo'].startswith('completato_'),
            'archived': bool(doc.get('_piercrew_source_id')) or doc['nome_torneo'].startswith('completato_')}
...
def save(store, original, expected_version, changes, *, collection=None):
    ...
    if '_piercrew_revision' not in original:
        conditions.append({'_piercrew_revision': {'$exists': False}})
    updated = deepcopy(changes)
    now = datetime.utcnow()
    updated['data_modifica'] = now.replace(microsecond=now.microsecond // 1000 * 1000)
    updated['_piercrew_revision'] = int(original.get('_piercrew_revision', 0)) + 1
