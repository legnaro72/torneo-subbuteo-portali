...
function SwissFlag(){return <span className="swiss-flag" aria-hidden="true"/>;}
...
function App() {
  ...
  return <div className="shell">
    <aside className="sidebar">
      <div className="brand-heading"><button className="brand" onClick={() => navigate('home')}><img className="brand-logo" src="/logo-superba.jpg" alt="Logo Superba"/><span>SUPERBA<small>SUBBUTEO CLUB</small></span></button><Signature/></div>
      <div className="nav-label">IL TUO MONDO</div>
      <nav aria-label="Navigazione principale">
        <button className={page === 'home' ? 'selected' : ''} aria-current={page==='home'?'page':undefined} onClick={() => navigate('home')}><LayoutDashboard size={19}/><span>Panoramica</span></button>
        <button className={page === 'club' ? 'selected' : ''} aria-current={page==='club'?'page':undefined} onClick={() => navigate('club')}><Users size={19}/><span className="nav-long">Gestione club</span><span className="nav-short">Club</span></button>
        <button className={page === 'italiana' ? 'selected' : ''} aria-current={page==='italiana'?'page':undefined} onClick={() => navigate('italiana')}><Trophy size={19}/><span className="nav-long">All’italiana</span><span className="nav-short">Italiana</span><span className="nav-dot"/></button>
        <button className={page === 'finali' ? 'selected' : ''} aria-current={page==='finali'?'page':undefined} onClick={() => navigate('finali')}><Flag size={19}/><span className="nav-long">Fasi finali</span><span className="nav-short">Finali</span></button>
        <button className={page === 'svizzero' ? 'selected' : ''} aria-current={page==='svizzero'?'page':undefined} onClick={() => navigate('svizzero')}><SwissFlag/><span>Svizzero</span></button>
      </nav>
    ...
    <main>
      ...
      <div className="content">
        ...
        {page==='club' ? <ClubView user={user} canWrite={canWrite} onDirty={setDirty} onLegacy={()=>external('club')}/> : (page==='finali'||page==='svizzero')?<CompetitionArea key={page} kind={page} user={user} canWrite={canWrite} directName={competitionName} onDirty={setDirty} onLegacy={()=>external(page)}/> : active ? <TournamentView key={active.id + ':' + user.id} tournament={active} user={user} canWrite={canWrite} onBack={() => navigate('italiana')} onSaved={value => {setActive(value);window.history.replaceState({},'',tournamentPath(value.name));void refresh();}} onDirty={setDirty} onLegacy={() => external('italiana-classica')} onFinali={() => navigate('finali')}/> : <>
          ...
          {page === 'home' && <div className="other-worlds"><div><span className="eyebrow">CONTINUA A GIOCARE</span><h2>Le altre competizioni</h2></div><button onClick={()=>navigate('finali')}><Flag/>Fasi finali<ArrowUpRight/></button><button onClick={()=>navigate('svizzero')}><SwissFlag/>Torneo svizzero<ArrowUpRight/></button></div>}
        </>}
        <footer>SUPERBA SUBBUTEO CLUB<span>Un piccolo campo, una grande passione.</span></footer>
      </div>
    </main>
