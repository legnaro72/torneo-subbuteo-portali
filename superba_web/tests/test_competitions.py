...
class CompetitionTests(unittest.TestCase):
    ...
    def test_final_inherits_premium_badges_from_preliminary(self):
        ...
        self.store.tournaments.update_one({'_id': ObjectId(self.source_id)}, {'$set': {'_superba_badges': {team: {'kind': 'flag', 'ref': 'IT'}}}})
        created = self.client.post('/api/finals', json={'source_id': self.source_id, 'mode': 'ko', 'qualifiers': 4,
                                                        'request_id': str(uuid4())})
        self.assertEqual(created.status_code, 200, created.text)
    ...
    def test_final_sources_accept_closed_original_without_completed_clone(self):
        ...
        original_id = str(self.store.tournaments.insert_one({'nome_torneo': 'Originale Chiuso',
            'calendario': calendar, 'data_creazione': datetime.utcnow(), '_superba_closed': True}).inserted_id)
        sources = self.client.get('/api/finals/sources')
