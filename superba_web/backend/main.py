...
def withdrawal(tournament_id: str, data: Withdrawal, user=Depends(writer), store: Store = Depends(store_dep)):
    ...
    return with_club_badges(store, view(save(store, doc, data.version, {'calendario': rows, '_superba_withdrawals': sorted(retired)})))
...
def save_badges(kind: str, tournament_id: str, data: SaveBadges, user=Depends(writer), store: Store = Depends(store_dep)):
    ...
    return with_club_badges(store, render(save(store, doc, data.version, {'_superba_badges': saved_badges}, collection=collection)))
