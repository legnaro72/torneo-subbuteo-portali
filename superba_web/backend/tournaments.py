...
def view(doc):
    ...
    standings = aggiorna_classifica(frame, doc.get('_superba_withdrawals', []))
    return {'id': str(doc['_id']), 'name': doc['nome_torneo'], 'version': version(doc), 'matches': rows,
            ...
            'withdrawals': doc.get('_superba_withdrawals', []),
            'closed': bool(doc.get('_superba_closed', False)) or doc['nome_torneo'].startswith('completato_'),
