...
class Store:
    ...
    def seed(self):
        ...
        for i, (name, team) in enumerate(names):
            self.players.insert_one({'Giocatore': name, 'Squadra': team, 'Potenziale': 10-i, 'Ruolo': 'A' if i == 0 else 'W', 'SetPwd': 1, 'Password': hash_password('superba-demo')})
        ...
        self.tournaments.insert_one({'_id': ObjectId('000000000000000000000001'), 'nome_torneo': 'Campionato Superba · Demo', 'calendario': calendar, 'data_creazione': datetime.utcnow(), '_superba_revision': 0})
        ...
        self.tournaments.insert_one({'_id': ObjectId('000000000000000000000003'), 'nome_torneo': 'completato_Torneo Superba · Demo', 'calendario': completed, 'data_creazione': datetime.utcnow(), '_superba_revision': 0})
        self.swiss_tournaments.insert_one({'_id': ObjectId('000000000000000000000002'), 'nome_torneo': 'Svizzero Superba · Demo', 'calendario': [], 'data_creazione': datetime.utcnow()})


@lru_cache
def get_store():
    if os.getenv('SUPERBA_DEMO', '').lower() == 'true':
        if os.getenv('VERCEL'):
