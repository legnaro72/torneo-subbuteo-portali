...
def player_view(doc, store=None):
    ...
    return {'id': str(doc['_id']), 'version': version(doc), 'name': str(doc.get('Giocatore') or ''),
            ...
            'password_set': doc.get('SetPwd') == 1, 'badge': badge_for_team(store, doc.get('Squadra')) if store else doc.get('_superba_badge'), **trophies}
...
def update_one(store, original, data, user):
    ...
    if data.badge is not None:
        changes['_superba_badge'] = data.badge.model_dump(exclude_none=True)
    ...
    if data.badge is not None:
        persist_club_badges(store, {data.team: changes['_superba_badge']})
...
def install(app, current_user, writer, store_dep):
    ...
    def create_player(data: PlayerInput, user=Depends(writer), store=Depends(store_dep)):
        ...
        if data.badge is not None:
            doc['_superba_badge'] = data.badge.model_dump(exclude_none=True)
        for count, names in TROPHIES:
            doc[count], doc[names] = 0, []
        result = store.players.insert_one(doc)
        doc['_id'] = result.inserted_id
        if data.badge is not None:
            persist_club_badges(store, {data.team: doc['_superba_badge']})
