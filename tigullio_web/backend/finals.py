"""Final stages compatible with the classic Tigullio calendar format."""
import hashlib
...
def view(doc):
    ...
    return dict(id=str(doc['_id']), name=doc['nome_torneo'], version=version(doc), matches=matches,
                ...
                active_round=current, finished=bool(doc.get('_tigullio_closed')) or doc['nome_torneo'].startswith('finito_'), winner=winner)
...
def award(store, doc, winner):
    ...
    if not isinstance(previous, list) or any(base_name(str(p)).casefold() == name.casefold() for p in previous):
        return
    store.players.update_one({'_id': player['_id'], '_tigullio_award_ids': {'$ne': f"ff:{doc['_id']}"}, 'listaFFElimDirettaVinte': {'$ne': name}},
                             {'$addToSet': {'listaFFElimDirettaVinte': name, '_tigullio_award_ids': f"ff:{doc['_id']}"}, '$inc': {'NFFElimDirettaVinte': 1}})
...
def install(app, current_user, writer, store_dep, require_tournament_write):
    ...
    def listing(user=Depends(current_user), store=Depends(store_dep)):
        result = []
        for doc in store.tournaments.find({'nome_torneo': {'$regex': '^(finito_)?fasefinale'}}, {'nome_torneo': 1, 'calendario': 1, '_tigullio_closed': 1}).sort('_id', -1):
            if is_final(doc):
                data = view(doc)
                result.append(dict(id=data['id'], name=data['name'], mode='ko', finished=data['finished']))
            elif is_italiana(doc):
                result.append(dict(id=str(doc['_id']), name=doc['nome_torneo'], mode='groups', finished=bool(doc.get('_tigullio_closed'))))
        return result
    ...
    def create(data: CreateFinals, user=Depends(writer), store=Depends(store_dep)):
        ...
        if existing:
            if existing.get('_tigullio_create_hash') != digest:
        ...
        doc = dict(_id=object_id, nome_torneo=name, calendario=rows, data_creazione=now, data_modifica=now,
                   _tigullio_badges={key: value for key, value in source.get('_tigullio_badges', {}).items() if key in qualifiers},
                   phase_metadata=dict(phase_id=str(object_id), phase_mode='KO' if data.mode=='ko' else 'GIRONI'),
                   _tigullio_preliminary_id=data.source_id, _tigullio_create_hash=digest, _tigullio_revision=0)
        try:
            store.tournaments.insert_one(doc)
        except DuplicateKeyError:
            existing = store.tournaments.find_one({'_id': object_id})
            if not existing or existing.get('_tigullio_create_hash') != digest:
    ...
    def advance(tournament_id: str, data: FinalAction, user=Depends(writer), store=Depends(store_dep)):
        ...
        if len(winners) == 1:
            saved = save(store, doc, data.version, {'nome_torneo': 'finito_'+doc['nome_torneo'], '_tigullio_closed': True})
