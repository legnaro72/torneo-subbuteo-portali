...
class Store:
    def __init__(self, players_client, tournaments_client, auth_client, *, demo=False):
        self.demo = demo
        self.players = players_client['giocatori_subbuteo']['tigullio_players']
        self.tournaments = tournaments_client['TorneiSubbuteo']['Tigullio']
        self.swiss_tournaments = tournaments_client['TorneiSubbuteo']['TigullioSvizzero']
        ...
        self.team_badges = auth['tigullio_team_badges']
    ...
    def seed(self):
        ...
        for i, (name, team) in enumerate(names):
            self.players.insert_one({'Giocatore': name, 'Squadra': team, 'Potenziale': 10-i, 'Ruolo': 'A' if i == 0 else 'W', 'SetPwd': 1, 'Password': hash_password('tigullio-demo')})
        ...
        self.tournaments.insert_one({'_id': ObjectId('000000000000000000000001'), 'nome_torneo': 'Campionato Tigullio · Demo', 'calendario': calendar, 'data_creazione': datetime.utcnow(), '_tigullio_revision': 0})
        ...
        self.tournaments.insert_one({'_id': ObjectId('000000000000000000000003'), 'nome_torneo': 'completato_Torneo Tigullio · Demo', 'calendario': completed, 'data_creazione': datetime.utcnow(), '_tigullio_revision': 0})
        self.swiss_tournaments.insert_one({'_id': ObjectId('000000000000000000000002'), 'nome_torneo': 'Svizzero Tigullio · Demo', 'calendario': [], 'data_creazione': datetime.utcnow()})


@lru_cache
def get_store():
    if os.getenv('TIGULLIO_DEMO', '').lower() == 'true':
        if os.getenv('VERCEL'):
