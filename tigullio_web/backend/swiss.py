"""Swiss tournaments in the original TigullioSvizzero document format."""
import hashlib
...
def award(store, doc):
    ...
    if not isinstance(previous, list) or any(str(value).removeprefix('finito_').strip().casefold() == name.casefold() for value in previous):
        return
    store.players.update_one({'_id': player['_id'], '_tigullio_award_ids': {'$ne': f"swiss:{doc['_id']}"}, 'listaCampionatiVinti': {'$ne': name}},
                             {'$addToSet': {'listaCampionatiVinti': name, '_tigullio_award_ids': f"swiss:{doc['_id']}"}, '$inc': {'NCampionatiVinti': 1}})
...
def install(app, current_user, writer, store_dep, require_tournament_write):
    ...
    def create(data: CreateSwiss, user=Depends(writer), store=Depends(store_dep)):
        ...
        doc = dict(_id=object_id, nome_torneo=name, df_torneo=[], df_squadre=teams, turno_attivo=1,
                   _tigullio_badges={key: badge.model_dump(exclude_none=True) for key, badge in data.badges.items()},
                   torneo_iniziato=True, torneo_finito=False, modalita_turni=data.mode, max_turni=data.max_rounds,
                   data_creazione=now, data_modifica=now, _tigullio_create_hash=digest, _tigullio_revision=0)
