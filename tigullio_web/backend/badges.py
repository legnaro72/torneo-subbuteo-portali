...
def club_badge_defaults(store, participants):
    ...
    if not wanted:
        return {}
    result = {}
    for player in store.players.find({'_tigullio_badge': {'$exists': True}}, {'Giocatore': 1, 'Squadra': 1, '_tigullio_badge': 1}):
        badge = player.get('_tigullio_badge')
